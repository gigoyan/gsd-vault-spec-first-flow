...
4. When the user supplies repository-relative files or directories that should become project evidence, route them through `$gsd-register-source-materials` before document-first extraction. Registration owns classification, custody, provenance, registry maintenance, claim indexing, conflicts/unknowns, and consumption logging; this skill consumes the registered material and extracts relevant claims into bootstrap and spec artifacts.
5. Determine whether the files are missing, placeholder-only, stale, or partially filled, then extract already-available facts from repo evidence, registered source materials, or transient supplied context before re-asking the user. For `document_first_intake`, extract into the Project Idea Document or Technical Specification templates too when the supplied material is already specific enough to support those artifacts.
6. Gather only the minimum missing inputs for the next required surfaces: objective, audience, goals, known stack or runtime context if already available, constraints, risks, non-goals, readiness gaps, and whether the project should adopt the shared Obsidian MCP vault, and if so resolve the `GSD Vault Project ID` from the repository root folder name unless `PROJECT.md` already contains a confirmed value. Do not require stack selection at bootstrap time.
7. Track extracted and requested inputs as `Confirmed`, `Suggested`, or `Unknown`. Treat repo-proven facts, registered source-backed facts, direct transient supplied-material facts, and explicit user decisions as `Confirmed`; keep derived recommendations or prefills as `Suggested`; keep unresolved or conflicting fields as `Unknown`.
8. When remaining user input is choice-shaped, use UI options rather than free-text whenever practical.
9. Rewrite or refresh the core bootstrap artifacts using the templates under [`.planning/templates/`](../../../.planning/templates/), including the bootstrap and adoption model for the project namespace under the shared Obsidian MCP vault root. When `PROJECT.md` contains `GSD-BLUEPRINT: project-surface-contract` and `GSD-PROJECT: project-charter` markers, update only the `GSD-PROJECT: project-charter` block and preserve the blueprint guidance block exactly. When the current project is already beyond placeholder bootstrap, update only the artifact surfaces that still need to be created or refreshed.
10. If the project is adopting the vault layer, record `GSD Vault Project ID`, `GSD Vault Namespace`, and `Vault scaffold status` in `PROJECT.md`, then create or verify `projects/<vault-project-id>/` through `gsd-vault-bootstrap` before milestone planning begins.
11. Record the next non-trivial readiness path explicitly: Project Idea Document -> preliminary technical direction -> `$gsd-select-stack` -> stack-aware Technical Specification completion -> milestone planning. Use the current project's filled-in spec artifacts for that path; do not treat [PROJECT.md](../../../PROJECT.md) or [`.planning/REQUIREMENTS.md`](../../../.planning/REQUIREMENTS.md) as substitutes.
12. Set [`.planning/STATE.md`](../../../.planning/STATE.md) to indicate bootstrap is complete and the next action is spec-readiness preparation rather than milestone planning when those governing artifacts are still missing or stale.
...
- When `document_first_intake` or `partial_maturity_continuation` reaches beyond bootstrap, refresh or create the current project's Project Idea Document and Technical Specification from extracted material instead of re-asking already answered questions. Cite registered `source_id` values and anchors when registered source materials support extracted claims.
...
- Route durable user-supplied repository-relative files or directories through `$gsd-register-source-materials` before document-first extraction when they should become project evidence.
- Extract from registered source materials or transient supplied context before re-asking the user when the intake route is `document_first_intake`.
- Do not duplicate source-material classification, custody, provenance, registry rows, or consumption logs inside bootstrap or spec artifacts.
