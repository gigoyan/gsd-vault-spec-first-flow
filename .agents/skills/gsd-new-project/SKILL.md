...
5. Gather only the minimum missing inputs for the next required surfaces: objective, audience, goals, known stack or runtime context if already available, constraints, risks, non-goals, readiness gaps, and whether the project-local vault scaffold needs to be created or verified. Do not require stack selection at bootstrap time.
...
8. Rewrite or refresh the core bootstrap artifacts using the templates under [`.planning/templates/`](../../../.planning/templates/), including the bootstrap and adoption model for the project-local vault. When the current project is already beyond placeholder bootstrap, update only the artifact surfaces that still need to be created or refreshed.
9. If the project is adopting the vault layer, create or verify the project-local vault scaffold through `gsd-vault-bootstrap` before milestone planning begins.
...
## Required Outputs
- [PROJECT.md](../../../PROJECT.md): project charter with objective, audience, goals, known stack or runtime context if already available, constraints, and non-goals, while keeping populated fields visibly `Confirmed`, `Suggested`, or `Unknown` when the template supports it.
